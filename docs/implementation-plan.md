...
- [x] 商品对存在任意来源记录时按实际数据生成报告。
- [x] 商品对五张来源表全部为空时跳过；整日无数据正常完成并在通知中统计。
...
- [x] DeepSeek 契约异常只重试当前分析，最终失败独立告警且不执行整体重试。
- [x] Healthchecks 接收任务开始、成功、失败及末尾错误日志。
- [x] 日周月批次成功后通过飞书群机器人 Webhook 发送完成卡片。
- [x] 运行状态文件和 API 提供当前阶段、商品对、完成数量与最近进度时间。
- [x] 进程锁冲突使用专用失败退出码，Healthchecks 不记录虚假成功。
...
- [x] 月报按自然月聚合已完成日报。
- [x] 数量和金额指标按日累加。
- [x] 转化率按累计成交人数除以累计访客数重新计算。
- [x] 客单价按累计成交金额除以累计成交人数重新计算。
- [x] 渠道、关键词、画像和推广占比根据累计值重新计算。
- [x] 周期内每天都有 ready 日报才生成，缺日记录日志并跳过；日均值按自然周期天数计算。
- [x] 周月报告保存来源日报 ID，并进入相同的 DeepSeek 分析流程。
- [x] 每天补查上一完整自然周、自然月，聚合事实相同的报告直接跳过。
- [x] 日周月独立执行并汇总通知，仅展示成功日期及份数，无成功结果时不发群消息。
