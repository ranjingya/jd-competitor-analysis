...
- [ ] 输入需要处理的日期。
- [ ] 从飞书读取本品 SPU 和竞品 SPU 候选组合。
- [ ] 使用 `<本品spu>+<竞品spu>` 生成 `compare_number`。
- [ ] 当天核心指标表找不到 `compare_number` 时直接跳过该组合。
- [ ] 当天核心指标存在时继续处理；其他来源缺失只标记对应模块不可用。
- [ ] 从飞书读取本品 SPU 下的 SKU。
- [ ] 从 StarRocks 读取五张竞品表和本品 SKU 日数据。
- [ ] 本品 SKU 数量、金额等可加总指标汇总到 SPU。
- [ ] 生成标准化数据集并写入数据库。
- [ ] 执行现有确定性分析并生成基础报告。
- [ ] 创建一条 AI 分析任务。
...
实现本品 SKU 日数据标准化与 SPU 汇总，并和五张竞品来源组装成完整日数据集。
