...
- 完成通知：日周月批次全部正常完成时，向群机器人发送绿色“报告更新”卡片，附生成时间（UTC+8）与“查看报告”链接。
- 失败通知：真正失败时向指定用户私聊发送告警，包含原因、时间、服务器、退出码和末尾日志摘要；失败时不发送群 Webhook。
- 无数据：记录处理结果，任务正常完成，不触发失败告警。

昨日日报固定展示：本次有生成时显示“成功 X 份”；没有新生成但已有报告时显示“报告已生成”；全部商品对无数据时显示“暂无数据”；没有该日期处理记录时显示“暂未生成”。昨天以脚本启动时的北京时间确定。历史日报、周报和月报只展示本次成功生成的日期及份数。

数据分析和 AI 分析生成成功均计为成功报告；整体重试按粒度、日期范围和商品对去重，保留本次已成功的结果。已有报告和无数据不计入成功份数。Healthchecks 接收最终执行状态。
