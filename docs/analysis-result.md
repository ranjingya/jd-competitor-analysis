...
基础分析流程只初始化空数组。Backend 把精简结构化事实写入 AI 执行记录，调用 DeepSeek 生成有证据支持的分析结果，校验结构后保存并合并到报告；Web 不直接写入该字段。
...
- 关键词覆盖汇总，以及按覆盖关系均衡选择的高影响关键词。
- 每个画像维度的主要完整差距和少量单侧数据样本。

AI 输入不包含：

- `tabs` 页面渲染结构。
- 商品图片地址。
- SKU ID、69 码、商品规格等 SKU 明细。
- 本品落区间校验、竞品候选估算过程等公式审计明细。
- 已有 AI 发现和建议。

关键词和画像输入同时携带完整行数与筛选后的代表性行。模型不得把未出现在代表性行中的项目判断为原始数据不存在。
