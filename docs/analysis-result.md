...
| 分析字段 | 标准化来源 | 转换规则 |
|---|---|---|
| `comparison[]` | `self_validation[]`、`competitor_core_conversions[]` | 计算带方向差值、差距幅度、百分点差和领先状态。 |
| `core_metrics[]` | `comparison[]` | 选择成交金额、访客数、成交转化率和成交客单价。 |

成交转化率使用“本品数据 − 竞品数据”的百分点差。规模和金额指标同时保存原单位差值，以及 `(本品数据 − 竞品数据) / 竞品数据` 的差距幅度百分比；正值不添加正号，负值保留减号。
...
| 两侧 `visitor_share` | 该侧关键词访客、该侧披露关键词访客合计 | 计算当前关键词在对应商品披露关键词中的访客占比。 |
...
- `validation`：实际执行后一周的复查指标，以执行前后可比周期和竞品变化为依据，不承诺提升幅度。

基础阶段及没有符合运营 SOP 的动作时使用空数组。建议数量和来源由实际触发结果决定，支持单条、单一来源及超过五条。网页只展示该数组，不根据数值、Tab 或固定句库生成建议。`warning` 为展示状态，不代表人工执行状态。

运营规则见 [运营分析 SOP](operations-sop.md)，模型运行规则位于 `backend/assets/ai-analysis-prompt.md`。触发判断与建议文案由 AI 按提示词生成，后端校验结构、字段和状态；人工执行记录及自动复查属于独立功能，不包含在报告契约中。
...
2. 来源或核心指标缺失时保留固定空结构，仅对现有事实执行分析，不推测缺失值。
