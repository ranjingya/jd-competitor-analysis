...
6. 页面按本品 SPU 分组，左侧下拉选择本品，右侧选择关联竞品；一至两个竞品使用商品卡，三个及以上使用带主图、按名称或 ID 搜索的下拉框。京东链接独立于选择按钮。
...
运行前端开发服务后，访问 `/test/product-navigation.html` 可执行隔离的浏览器回归测试。页面使用模拟 API，验证本品分组、竞品搜索、同周期空报告、快速切换响应隔离、键盘操作及 320、900、1280 像素布局，不读取或修改业务数据。
