...
分析请求按 `DEEPSEEK_MODEL` 选择模型，思考模式、推理强度和输出长度使用模型默认值，
仍受服务端限制。请求指定 JSON 输出格式，并校验摘要、要点和建议结构。
请求超时默认 300 秒，每次分析最多请求 2 次，网络异常与结果校验失败共享尝试次数。
费用使用空闲时段单价估算；定时任务应安排在官方定义的空闲时段执行。
...
LARK_COMPLETION_WEBHOOK_URL=<飞书群机器人 Webhook 地址>
...
应用需要获得目标多维表格的只读文档权限，并在开发者后台开通读取多维表格记录和 `im:message:send_as_bot` 权限。通知接收人需要位于应用可用范围内，并已与机器人建立可发送消息的关系。`LARK_ALERT_OPEN_ID` 必须由当前 `LARK_APP_ID` 查询得到。映射流程只调用获取应用凭证和列出记录接口，不调用多维表写入、更新或删除接口；宿主机脚本在整个日周月批次成功后通过群机器人 Webhook 发送一张绿色完成卡片，在最终失败时通过自建应用向指定用户发送一张单聊告警卡片。
...
商品对表每行配置一个 `本品spu` 和多个编号竞品列：`竞品spu1`、`竞品spu2`、`竞品spu3` 等。程序分页读取记录，自动识别编号列，将一行展开成独立的本品与竞品组合。空竞品跳过；非法 ID 或本竞品相同只跳过对应竞品，不影响同一行其他有效竞品；组合按两个 SPU 去重。`本品品名`、`本品图片` 和竞品图片列不参与该解析，主图由 `data/product-images.json` 维护。

每个组合独立生成日、周、月报告，报告唯一键包含本品和竞品 SPU。同一本品可对应多个竞品，同一个竞品也可对应不同本品。页面仅展示已经有报告的组合。
...
周期内每个自然日都必须存在已完成日报；任意一天缺失、AI 失败或未完成时，记录缺失日期并正常跳过该商品对。日报的个别指标为空不影响此门槛。已有周月报在聚合业务事实相同时直接跳过，事实发生变化时更新同一份报告。

定时脚本每天依次检查最近七天日报、上一完整自然周和上一完整自然月，各阶段独立执行，最终统一上报失败或成功。更早周期使用手动命令；周月报不会主动从数仓补日报。
...
来源表、商品角色或指标缺失时保持固定空结构，质量状态为 `partial`，现有数据继续进入确定性分析和 DeepSeek。字段值为 `null`、`masked` 或 `0`，以及双方维度项不完全一致，均按数仓事实处理。商品对在五张表中全部没有记录时跳过报告；同一业务日期的全部商品对均没有记录时记录整日无数据，任务正常结束。

日报批次按以下方式处理运行异常：

- 单个商品对五张来源表全部为空：跳过当前商品对，不立即重试，后续七天缺口检查再次读取。
- 主业务日期的全部商品对均为空：记录无数据，正常完成七天检查，不触发失败告警，计划中的周月报继续执行。
