...
| 目录 | 内容 |
|---|---|
| `assets/` | 会被脚本直接消费的商品素材、JSON 空结构和回归示例。 |
...
| 文件 | 用途 |
|---|---|
| `assets/product-images.json` | 按商品 ID 保存展示名称和京东 HTTPS 主图地址，供报告生成阶段写入商品引用。 |
...
| `scripts/jd_competitor_analysis/product_assets.py` | 加载并校验商品主图素材，组装报告中的本品与竞品商品引用。 |
