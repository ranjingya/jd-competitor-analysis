...
4. 按 `references/analysis-result.md` 和 `references/field-map.md` 生成并校验事实层 `analysis_result.json`；此时 `ai_recommendations` 保持空数组。
5. 读取完整 `analysis_result.json` 与 `references/recommendation-rules.md`，独立分析指标之间的结构关系，生成 1–5 条 `ai_recommendations`，再用 `scripts/apply_ai_recommendations.py` 写回当前报告。
6. 批量模式按粒度目录发现周期，每周期独立生成两份 JSON，并在全部周期完成后写入 `report-index.json`；需要正式展示 AI 建议的周期逐一执行建议分析步骤。
7. 检查核心必需数据、完整看板数据和诊断增强数据的可用状态；缺失项按映射规则降级并写入 `risks[]`。
8. 按 `references/dashboard.md` 检查日、周、月切换、周期选择、首屏、三个差距来源 Tab、AI 核心判断与风险提示。
9. 原始 Excel 全程只读，计算只在分析层完成，网页按需消费一份 `analysis_result.json`，不在前端生成建议。
...
| `references/recommendation-rules.md` | 需要生成或维护流量、关键词、客户画像的可执行建议和禁用规则时读取。 |
