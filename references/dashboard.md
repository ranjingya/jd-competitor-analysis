...
3. 四张指标卡在桌面端位于趋势图左侧纵向排列，使用左侧色条和浅色背景表达状态；本品与竞品数值使用一致字号。规模和金额指标在卡片顶部展示“差值、分隔线、差距幅度百分比”，成交转化率只展示百分点差；正值不添加正号，负值保留减号。窄屏按 2×2 和单列自适应。
...
5. 每个差距来源 Tab 先展示 3 至 5 项重点数据，再展示完整表格。重点卡明确标注差值对应的指标名称；规模指标展示相对竞品的差距幅度百分比，结构占比展示百分点差。
