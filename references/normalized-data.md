...
4. 月维度表内时间支持 `YYYYMM`，并与周期起止月份进行一致性校验。
5. `core_raw` 和 `self_real` 在正式分析中必须唯一。
6. `-`、空值和无法解析值原样保留，不转换成 `0`。
7. SPU、周期或文件角色无法唯一匹配时写入 `warnings`；核心数据冲突时停止正式分析。
8. 区间解析、P 候选、约束校正和业务判断全部留到分析阶段。
9. 分析规则调整后，允许使用同一份标准化事实数据重新生成分析结果。
